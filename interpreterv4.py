...
class Interpreter(InterpreterBase):
    ...
    def __get_met_by_name(self, obj_name, name, num_params):
        if name not in self.func_name_to_ast:
            obj_val = self.env.get(obj_name)
            if obj_val is None:
                super().error(ErrorType.NAME_ERROR, "idk")
            obj = obj_val.value()
            obj_env = obj.captured_env

            # has parent
            if obj.parent is not None:
                parents = []
                temp_obj = obj
                while temp_obj.parent is not None:
                    parents.append(temp_obj.parent)
                    temp_obj = temp_obj.parent

                closure_val_obj = obj_env.get(name)

                if closure_val_obj is None:
                    for parent in parents:
                        if parent.captured_env.get(name) is not None:
                            closure_val_obj = parent.captured_env.get(name)
    ...
    def __call_method(self, call_ast):
        ...
        if obj_name == "this":
            obj_val = self.this_obj[len(self.this_obj) - 1]
            obj_name = self.this_obj[len(self.this_obj) - 1].value().name
        else:
            obj_val = self.env.get(obj_name)

        if obj_val is None:
            super().error(
                ErrorType.NAME_ERROR, f"Function {obj_name}.{func_name} not found"
            )
        if obj_val.type() != Type.OBJECT:
            super().error(ErrorType.TYPE_ERROR, f"non obj")

        self.this_obj.append(obj_val)
        obj = obj_val.value()
        obj_env = obj.captured_env

        target_closure = self.__get_met_by_name(obj_name, func_name, len(actual_args))
    ...
    def __assign(self, assign_ast):
        ...
        if var_name == "this":
            super().error(ErrorType.NAME_ERROR, f"this something error obj")

        src_value_obj = copy.copy(self.__eval_expr(assign_ast.get("expression")))

        # check if field of object
        if "." in var_name:
            obj_name = var_name[0 : var_name.index(".")]
            f_name = var_name[var_name.index(".") + 1 :]
            if obj_name == "this":
                obj_val = self.this_obj[len(self.this_obj) - 1]
            else:
                obj_val = self.env.get(obj_name)
            ...
            # print(obj_val.type())
            # if obj_val.type() != Type.OBJECT:
            #     super().error(ErrorType.TYPE_ERROR, "not obj type")
            if obj_val is None:
                super().error(ErrorType.NAME_ERROR, "no obj found")
            if obj_val.type() != Type.OBJECT:
                super().error(ErrorType.TYPE_ERROR, "not obj type")
            obj = obj_val.value()
            obj.name = obj_name
            ...
            if f_name == "proto":
                if src_value_obj.type() == Type.NIL:
                    super().error(ErrorType.NAME_ERROR, "not obj nil")
                if src_value_obj.type() != Type.OBJECT:
                    super().error(ErrorType.TYPE_ERROR, "not obj type")
                obj.parent = src_value_obj.value()
                # print("*******************************")
                # print(assign_ast.get("expression"))
                return

            obj_env = obj.captured_env
            # obj_env.set(f_name, src_value_obj)

            ntarget_value_obj = obj_env.get(f_name)
            # print()
            # print("******************************")
            # print(obj_name + "---" + f_name)
            # print(ntarget_value_obj)
            # print(src_value_obj.value())
            # print("******************************")

            if ntarget_value_obj is None:
                obj_env.set(f_name, src_value_obj)

            else:
                # if a close is changed to another type such as int, we cannot make function calls on it any more
                if (
                    ntarget_value_obj.t == Type.CLOSURE
                    and src_value_obj.t != Type.CLOSURE
                ):
                    ntarget_value_obj.v.type = src_value_obj.t
                ntarget_value_obj.set(src_value_obj)
    ...
    def __eval_expr(self, expr_ast):
        ...
        if expr_ast.elem_type == "@":
            self.this_obj.append(Value(Type.OBJECT, Object(None, EnvironmentManager())))
            return Value(Type.OBJECT, Object(None, EnvironmentManager()))

    def __eval_name(self, name_ast):
        var_name = name_ast.get("name")
        if "this" == var_name:
            return self.this_obj[len(self.this_obj) - 1]
        if "." in var_name:
            obj_name = var_name[0 : var_name.index(".")]
            f_name = var_name[var_name.index(".") + 1 :]
            ...
            if f_name == "proto":
                ...
                if obj.parent is None:
                    super().error(ErrorType.NAME_ERROR, f"got the milk")
                return Value(Type.OBJECT, obj.parent)
            ...
            if val is None:
                ...
                if obj.parent is not None:
                    ...
                    closure_val_obj = obj_env.get(f_name)

                    if closure_val_obj is None:
                        for parent in parents:
                            if parent.captured_env.get(f_name) is not None:
                                closure_val_obj = parent.captured_env.get(f_name)
                if closure_val_obj is not None:
                    return closure_val_obj
                else:
                    super().error(ErrorType.NAME_ERROR, f"Variable/functio not found")
